...
import { getWalletWithProvider, getNftsForOwner, getLink } from "../utils"
...
export const handler = async (argv: Arguments<Options>): Promise<void> => {
  ...
  try {
    const out = await answer(privateKey, chain, providerUrl)
    console.log(out)
    process.exit(0)
  } catch (err: any) {
...
export const answer = async (
  ...
  return JSON.stringify({ hash: tx.hash, link }, null, 2)
}
